...
{
    /// <summary>
    /// An implementation of the C standard library - stdlib.h
    /// for use by the converted code.
    /// </summary>
    public static unsafe class CRuntime
    {
        ...
        {
            var spanA = new Span<byte>(a, (int) size);
            var spanB = new Span<byte>(b, (int) size);
            spanA.CopyTo(spanB);
        }
